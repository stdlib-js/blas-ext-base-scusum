...
var scusum = tryRequire( resolve( __dirname, './../lib/ndarray.native.js' ) );
var opts = {
...
tape( 'main export is a function', opts, function test( t ) {
	...
	t.strictEqual( typeof scusum, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 8', opts, function test( t ) {
	t.strictEqual( scusum.length, 8, 'has expected arity' );
	t.end();
});
...
tape( 'the function calculates the cumulative sum', opts, function test( t ) {
	...
	scusum( x.length, 0.0, x, 1, 0, y, 1, 0 );
	...
	scusum( x.length, 10.0, x, 1, 0, y, 1, 0 );
	expected = new Float32Array([
		11.0,
		13.0,
		16.0,
...
tape( 'the function returns a reference to the output array', opts, function test( t ) {
	...
	out = scusum( x.length, 0.0, x, 1, 0, y, 1, 0 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `y` unchanged', opts, function test( t ) {
	...
	scusum( -1, 0.0, x, 1, 0, y, 1, 0 );
	t.deepEqual( y, expected, 'returns `y` unchanged' );

	scusum( 0, 0.0, x, 1, 0, y, 1, 0 );
	t.deepEqual( y, expected, 'returns `y` unchanged' );
...
tape( 'the function supports negative strides', opts, function test( t ) {
	...
	scusum( N, 0.0, x, -2, x.length-1, y, -1, 2 );
